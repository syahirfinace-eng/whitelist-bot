...
const CONFIG = {
  ...
  // Pterodactyl Panel — untuk akses file log MC server
  PTERO_URL: process.env.PTERO_URL || "",          // contoh: https://panel.zelpstore.com
  PTERO_KEY: process.env.PTERO_KEY || "",          // API Key dari panel
  PTERO_SERVER_ID: process.env.PTERO_SERVER_ID || "", // ID server (contoh: 41ab30e2)
...
// =====================================================
// PTERODACTYL API — baca & tulis file log MC server
// =====================================================

// Baca isi file dari panel
async function pteroReadFile(filePath) {
  const url = `${CONFIG.PTERO_URL}/api/client/servers/${CONFIG.PTERO_SERVER_ID}/files/contents?file=${encodeURIComponent(filePath)}`;
  const res = await fetch(url, {
    headers: {
      "Authorization": `Bearer ${CONFIG.PTERO_KEY}`,
      "Accept": "application/json",
    },
  });
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`Ptero read error: ${err}`);
  }
  return await res.text();
}

// Tulis/overwrite isi file ke panel
async function pteroWriteFile(filePath, content) {
  const url = `${CONFIG.PTERO_URL}/api/client/servers/${CONFIG.PTERO_SERVER_ID}/files/write?file=${encodeURIComponent(filePath)}`;
  const res = await fetch(url, {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${CONFIG.PTERO_KEY}`,
  ...
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`Ptero write error: ${err}`);
  }
}

// Hapus baris yang mengandung username tertentu dari log
async function removeFromLog(username) {
  const content = await pteroReadFile(CONFIG.LOG_FILE_PATH);
  ...
  await pteroWriteFile(CONFIG.LOG_FILE_PATH, newContent ? newContent + "\n" : "");
...
` +
  ...
  if (content.startsWith("!resetlog")) {
    ...
    if (!CONFIG.PTERO_URL || !CONFIG.PTERO_KEY || !CONFIG.PTERO_SERVER_ID) {
      return message.reply("❌ Pterodactyl belum dikonfigurasi. Tambahkan `PTERO_URL`, `PTERO_KEY`, dan `PTERO_SERVER_ID` di env Railway.");
    }
