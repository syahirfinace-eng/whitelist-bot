...
const CONFIG = {
  ...
  // Label form Java (Ticket Tool panel Java)
  FORM_FIELD_JAVA: "Apa Nama Username MC kamu",

  // Label form Bedrock (Ticket Tool panel Bedrock)
  FORM_FIELD_BEDROCK: "Apa Nama Username Bedrock kamu",
...
async function whitelistPlayer(username, isBedrock) {
  ...
  try {
    ...
    let command;
    if (isBedrock) {
      // Bedrock player — tambah titik di depan, pakai fwhitelist
...
// Return: { username, isBedrock } atau null
function parseUsernameFromEmbed(embeds) {
  for (const embed of embeds) {
    ...
    for (const field of embed.fields) {
      ...
      if (fieldName.includes(CONFIG.FORM_FIELD_BEDROCK.toLowerCase())) {
        return { username: field.value?.trim(), isBedrock: true };
      }
      if (fieldName.includes(CONFIG.FORM_FIELD_JAVA.toLowerCase())) {
        return { username: field.value?.trim(), isBedrock: false };
      }
    }
    ...
    if (embed.description) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (lineLower.includes(CONFIG.FORM_FIELD_BEDROCK.toLowerCase())) {
          const val = lines[i + 1]?.replace(/[*_`]/g, "").trim();
          if (val) return { username: val, isBedrock: true };
        }
        if (lineLower.includes(CONFIG.FORM_FIELD_JAVA.toLowerCase())) {
          const val = lines[i + 1]?.replace(/[*_`]/g, "").trim();
          if (val) return { username: val, isBedrock: false };
        }
      }
    }
  }
...
client.on(Events.MessageCreate, async (message) => {
  ...
  const parsed = parseUsernameFromEmbed(message.embeds);
  ...
  console.log(`🎮 Username terdeteksi: ${displayName} (${platform})`);
